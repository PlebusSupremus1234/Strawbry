    ...
    if (IS_OBJ(callee)) {
        switch (OBJ_TYPE(callee)) {
            ...
            case OBJ_CLASS: {
                ...
                Value initializer;
                if (tableGet(&_class->methods, vm.initString, &initializer)) return call(AS_CLOSURE(initializer), argCount);
                else if (argCount != 0) {
...
static bool isFalsey(Value value) { return IS_NULL(value) || (IS_BOOL(value) && !AS_BOOL(value)); }
    ...
    for (;;) {
        ...
        switch (instruction = READ_BYTE()) {
            ...
            case OP_GET_PROPERTY: {
                ...
                if (!bindMethod(instance->_class, name)) return INTERPRET_RUNTIME_ERROR;
                break;
            }
