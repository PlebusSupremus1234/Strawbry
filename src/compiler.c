...
static uint8_t argumentList() {
    ...
    if (!check(TOKEN_RIGHT_PAREN)) {
        do {
            ...
            if (argCount == 255) error("Cannotthave more than 255 arguments");
...
static void _super(bool canAssign) {
    ...
    consume(TOKEN_DOT, "Expected a '.' after 'super'");
...
static void function(FunctionType type) {
    ...
    if (!check(TOKEN_RIGHT_PAREN)) {
        do {
            ...
            if (current->function->arity > 255) errorAtCurrent("Function cannot have more than 255 parameters");
            uint8_t constant = parseVariable("Expected a parameter name");
            defineVariable(constant);
        } while (match(TOKEN_COMMA));
    }

    consume(TOKEN_RIGHT_PAREN, "Expected a ')' after parameters");
...
static void method() {
    consume(TOKEN_FN, "Expected a 'fn' keyword");
...
static void forStatement() {
    ...
    if (!match(TOKEN_RIGHT_PAREN)) {
        ...
        expression();
        emitByte(OP_POP);
        consume(TOKEN_RIGHT_PAREN, "Expected a ')' after for clauses");
...
static void printStatement() {
    consume(TOKEN_LEFT_PAREN, "Expected a '(' after print function");
    expression();
    consume(TOKEN_RIGHT_PAREN, "Expected a ')' after print function arguments");
    emitByte(OP_PRINT);
}
...
static void declaration() {
    ...
    else if (match(TOKEN_FN)) funcDeclaration();
    else if (match(TOKEN_VAR)) varDeclaration();
    else statement();
